...
// 1. Only Classic Consumer Groups should be listed with Classic Group Type Option
// 2. Our Created Consumer Group should be listed with Consumer Group Type Option
func (its *IntegrationTestSuite) TestAdminClient_ListConsumerGroups() {
	t := its.T()
	if testConsumerGroupProtocolClassic() {
		t.Skipf("KIP 848 ListConsumerGroups needs Consumer Protocol to verify the updated changes")
	}
	rand.Seed(time.Now().Unix())
	groupID := fmt.Sprintf("%s-%d", testconf.GroupID, rand.Int())
	...
	if findConsumerGroupListing(groups, groupID) != nil {
		t.Errorf("Consumer groups %s should not be present\n", groupID)
		return
	}
	...
	groups = listGroupResult.Valid
	for _, groupInfo := range groups {
		if groupInfo.GroupType != ConsumerGroupTypeClassic {
			fmt.Printf("GroupName : %s, GroupType : %d", groupInfo.GroupID, groupInfo.GroupType)
			t.Errorf("The Consumer Group Type should be Classic as the Classic Option was set\n")
			return
		}
	}
	if findConsumerGroupListing(groups, groupID) != nil {
		t.Errorf("Consumer group %s should not be present as the Group Type Option was set to Classic\n", groupID)
		return
	}
